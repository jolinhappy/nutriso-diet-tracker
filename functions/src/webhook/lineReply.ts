...
// ✅ 已記錄午餐
// - 雞胸肉 150g → 蛋白質 35g / 碳水 0g / 脂肪 3g
// ...
// 今日剩餘
// 熱量：1575 kcal
// ...
// 詳細資訊請點選下方選單「今日紀錄」
export function buildSuccessReply(meal: AiParsedMeal, summary: DailySummary): string {
  const itemLines = meal.items
    .map((item) => `- ${item.name} ${item.amount} → 蛋白質 ${item.protein}g / 碳水 ${item.carbs}g / 脂肪 ${item.fat}g`)
    .join('\n')
